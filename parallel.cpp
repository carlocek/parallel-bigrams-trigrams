...
//	cout << wchunksize << endl;
	...
	#pragma omp parallel default(none) shared(text, textLength, wordsReduction, wordsLength, bcFreq, tcFreq, bwFreq, twFreq)
	{
		...
		bcstart = tid*(textLength/nth);
		tcstart = tid*(textLength/nth);
		bwstart = tid*(wordsLength/nth);
		twstart = tid*(wordsLength/nth);
		if(tid == nth-1)
		{
			bcend = ((tid+1)*(textLength/nth))-1;
			bwend = ((tid+1)*(wordsLength/nth))-1;
			tcend = ((tid+1)*(textLength/nth))-2;
			twend = ((tid+1)*(wordsLength/nth))-2;
		}
		else
		{
			bcend = (tid+1)*(textLength/nth);
			bwend = (tid+1)*(wordsLength/nth);
			tcend = (tid+1)*(textLength/nth);
			twend = (tid+1)*(wordsLength/nth);
		}
